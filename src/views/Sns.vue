<template>
  <div>
    ...
    <div v-if="snsFlag" class="md-layout md-gutter md-alignment-center">
      <md-card v-for="item in sns" v-bind:key="item.title" style="margin: 50px 20px 20px 20px;width: 30%;">
        <md-card-header>
          <div class="md-title">
            <a v-bind:href="item.url">{{item.key}}</a>
          </div>
        </md-card-header>
        <md-card-content>
          {{item.text}}
